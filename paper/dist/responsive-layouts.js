function ResponsiveLayouts(class_btn,text_left,text_right) {
	...
	if(!text_left){text_left='left';}
	if(!text_right){text_right='right';}
	if(!class_btn){class_btn='';}
	...
	if ($('aside.right').length) {
		if (!$('.sidebtnright').length) {
			$('aside.right').prepend('<button class="sidebtnright '+class_btn+'">'+text_right+'</button>');
		}
	...
	if ($('aside.left').length) {
		if (!$('.sidebtnleft').length) {
			$('aside.left').prepend('<button class="sidebtnleft '+class_btn+'" >'+text_left+'</button>');
		}
	...
	$(document).on('click', '.panel-heading > .overlay', function (e) {
	    var $this = $(this);
...
if ($(window).width() < 861) {diff=25;}
	...
	$('aside.left section').css("height", function(){calcy= calcx -rel_header; return  calcy+'px'});
	$('aside.right section').css("height", function(){calcy= calcx -rel_right; return  calcy+'px'});
}
